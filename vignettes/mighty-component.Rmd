...
has_pharmaverse <- requireNamespace("pharmaverseadam", quietly = TRUE) &&
...
Because the component code assigns back to `{{ domain }}`, calling `$eval()`
modifies the domain variable in your environment directly — no assignment of the
return value is needed.

```{r eval-setup, eval=has_pharmaverse}
...
```{r eval-run, eval=has_pharmaverse}
rendered$eval()
names(adae)
head(adae)
```
...
```{r custom-eval, eval=has_pharmaverse}
...
```{r test-create, eval=has_pharmaverse}
...
```{r test-assign, eval=has_pharmaverse}
...
```{r test-eval, eval=has_pharmaverse}
test$eval()
...
```{r test-coverage, eval=has_pharmaverse}
...
modifies columns, and passes it along. Here is an example that builds up an
ADAE step by step from SDTM data.

```{r chain-setup, eval=has_pharmaverse}
...
```{r chain-derive, eval=has_pharmaverse}
...
```{r chain-result, eval=has_pharmaverse}
adae |>
  dplyr::select(
